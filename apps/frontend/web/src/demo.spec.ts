...
describe('Validation component', () => {
    it('renders the description', () => {
        const { getByText } = render(Validation, { props: { description: 'Must be at least 8 characters' } });
...
describe('Input component', () => {
    it('renders the input with the correct placeholder', () => {
        const { getByPlaceholderText } = render(Input, { props: { placeholder: 'Enter your username', title: 'Username', value: '' } });
...
describe('Layout Component', () => {
    it('renders the correct heading based on the URL', () => {
        const { getByText } = render(Layout, { props: { $page: { url: { pathname: '/auth/signup' } } } });
        expect(getByText('Create an account')).toBeTruthy();

        const { getByText: getByTextLogin } = render(Layout, { props: { $page: { url: { pathname: '/auth/login' } } } });
