<script lang="ts">
    ...
    async function handleSubmit(event: any): Promise<any> {
        ...
        try {
            ...
            const { data, errors } = await query.mutate({ data: {
                email: email,
                password: password,
                username: username,
                firstName: firstname,
...
<form onsubmit={handleSubmit} class="w-full flex flex-col gap-2">
    ...
    <Input bind:value={password} title="Password" placeholder="Create your password" dataType="password" />
    ...
    <p>
        Already have an account?<a href="/auth/login/" class="font-semibold pl-2">Log in</a>
    </p>
</form>